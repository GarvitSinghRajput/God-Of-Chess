        ...
        if (waiting_end_state)
        {
            if (wait_for_anim_start)
            {
                ...
                if (animator_obj.GetCurrentAnimatorStateInfo(0).IsName(states[tempparamnumber].endStateName))
                {
                    ...
                    if (ArrowObj && tempparamnumber < states.Length - 1 && Math.Round(normalizedTimeInCurrentLoop) >= arrowTime[tempparamnumber])
                    {
                        ...
                        if (!test)
                        {
                            ...
                            {
                                vfxDetails[tempparamnumber].effects[i].transform.position = bowObj.transform.GetChild(1).position;
                                vfxDetails[tempparamnumber].effects[i].SetActive(true);
