...
public class Board : MonoBehaviour
{
    ...
    private void MoveChessman(int x, int y)
    {
        if (allowedMoves[x, y])
        {
            ...
            selectedChessman.transform.position = GetTileVector(x, y);
    ...
    private void SpawnChessman(int index, int x, int y)
    {
        ...
        GameObject go = Instantiate(chessmanPrefabs[index], GetTileVector(x, y), orientation);
        go.AddComponent<Outline>();
        go.GetComponent<Outline>().OutlineWidth = 8f;
    ...
    private Vector3 GetTileVector(int x, int y)
    {
